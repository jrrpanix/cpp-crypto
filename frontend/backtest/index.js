...
async function runMultiSymbolBacktest(event) {
    ...
    try {
        ...
        if (downThreshold >= 0) {
            showStatus('Down threshold must be negative', 'error');
            return;
        }
...
function createCumulativePnlChart() {
    ...
    labels.unshift(new Date(sortedTimes[0] - 60000)); // 1 minute before first trade
    ...
    cumulativeChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: `Portfolio Cumulative Net PnL Over Time (${currentResults.length} Symbols)`,
                    font: {
                        size: 16,
                        weight: 'bold'
                    }
