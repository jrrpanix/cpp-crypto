...
(function() {
    ...
    async function fetchSymbols() {
        try {
            const response = await fetch(`${API_BASE_URL}/api/daily-symbols`);
            const data = await response.json();
            
            if (data.success && data.symbols) {
                availableSymbols = data.symbols;
                console.log(`Loaded ${availableSymbols.length} available daily symbols`);
            } else {
    ...
    async function runMultiSymbolBacktest(event) {
        ...
        try {
            ...
            showStatus(`Running daily backtest on ${selectedSymbols.length} symbols...`, 'info');
