...
def load_daily_klines(klines_dir: Path, start_date: datetime, end_date: datetime, suffix: str = "USDT") -> pl.DataFrame:
    """
...
def calculate_adv_weights(
    ...
    if units == "months":
        ...
        if interval == 1:
            ...
            result = result.drop(["next_year", "next_month", "day_count", "month_days", "year", "month"])
...
def build_index_returns(
    ...
    result = portfolio_returns.select([
        "open_time", "index_level", "num_symbols", "portfolio_return",
        "synthetic_volume", "synthetic_quote_volume"
    ])
...
def convert_to_kline_format(index_df: pl.DataFrame, index_symbol: str) -> pl.DataFrame:
    ...
    kline_df = kline_df.with_columns([
        pl.lit(index_symbol).alias("symbol")
    ])
...
Examples:
  ...
  python build_index.py --klines-dir /workspace/data/klines_daily \\
    ...
    parser.add_argument("--klines-dir", type=str, required=True,
    ...
    parser.add_argument("--top-n", type=int, default=10,
                        help="Number of top symbols to include (default: 10)")
    parser.add_argument("--drop-n", type=int, default=0,
                        help="Number of top symbols to drop (default: 0)")
    parser.add_argument("--interval", type=int, default=1,
                        help="Rebalance interval length (default: 1)")
    parser.add_argument("--units", type=str, default="months", choices=["months", "weeks"],
                        help="Rebalance interval units (default: months)")
    parser.add_argument("--suffix", type=str, default="USDT",
                        help="Symbol suffix filter (default: USDT)")
    parser.add_argument("--symbol", type=str, required=True,
                        help="Index symbol name (e.g., IX10, IX25, TOPX)")
    parser.add_argument("--name", type=str,
                        help="Index display name (default: uses --symbol)")
    parser.add_argument("--output", type=str,
                        help="Output parquet file path (default: <klines-dir>/<symbol>.parquet)")
    parser.add_argument("--base-level", type=float, default=100.0,
                        help="Starting index level (default: 100)")
    parser.add_argument("--plot", action="store_true",
                        help="Generate plot of index")
    parser.add_argument("--plot-file", type=str,
                        help="Output file for plot (default: <symbol>_plot.png)")
