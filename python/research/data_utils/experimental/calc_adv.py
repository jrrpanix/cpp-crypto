...
def _calculate_adv_months(
    df: pl.DataFrame, interval_months: int, top_n: int = None, drop_n: int = 0
) -> pl.DataFrame:
    ...
    if interval_months == 1:
        ...
        result = result.with_columns(
            [
                (pl.date(pl.col("next_year"), pl.col("next_month"), 1) - pl.duration(days=1)).alias(
                    "end_date"
                )
            ]
        )
        ...
        df = df.with_columns(
            [
                ((pl.col("year") * 12 + pl.col("month") - 1) / interval_months)
                .floor()
        ...
        result = result.filter(
            pl.col("day_count") >= (pl.col("interval_days") * 0.9).cast(pl.Int64)
        )
...
def _calculate_adv_weeks(
    ...
    if index_start_day:
        ...
        if target_weekday is None:
            raise ValueError(
                f"Invalid day: {index_start_day}. Use monday, tuesday, wednesday, thursday, friday, saturday, or sunday"
            )
        ...
        df = df.with_columns(
            [
                ((pl.col("open_time").cast(pl.Date) - pl.lit(anchor_date)).dt.total_days() / 7)
                .floor()
                .alias("weeks_since_start")
            ]
        )
        ...
        df = df.with_columns(
            [
                (
                    (pl.col("open_time").cast(pl.Date) - pl.lit(first_month_start)).dt.total_days()
                    / 7
                )
                .floor()
                .alias("weeks_since_start")
        ...
        df = df.with_columns(
            [
                (
                    (pl.col("open_time").cast(pl.Date) - pl.lit(first_date.date())).dt.total_days()
                    / 7
                )
                .floor()
                .alias("weeks_since_start")
...
def plot_adv(df: pl.DataFrame, output_path: Path, top_symbols: int = 10):
    ...
    ax1.set_title(
        f"Top {top_symbols} Symbols by Average Daily Volume Over Time",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
    ...
    ax2.stackplot(
        pivot_df.index,
        *[pivot_df[col] / 1e9 for col in pivot_df.columns],
    ...
    ax2.set_title(
        f"Market Share: Top {top_symbols} Symbols ADV (Stacked)",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def plot_weights(df: pl.DataFrame, output_path: Path, num_intervals: int = 20):
    ...
    pivot_df = plot_df.pivot_table(
        index="begin_date", columns="symbol", values="weight", fill_value=0
    )
    ...
    ax.set_title(
        "Symbol Weights Over Time (Each Bar Sums to 1.0)", fontsize=14, fontweight="bold", pad=20
    )
    ...
    # Format x-axis
    labels = [d.strftime("%Y-%m-%d") if isinstance(d, datetime) else str(d) for d in pivot_df.index]
...
Examples:
  ...
  python calc_adv.py --input-file AGG_2024-07-01_2025-09-30.pq --interval 1 --units weeks --nsymbols 25 --output-dir /workspace/data --plot
    ...
    parser.add_argument(
        "--input-file", type=str, required=True, help="Path to aggregate parquet file"
    )
    parser.add_argument("--interval", type=int, default=1, help="Interval length (default: 1)")
    parser.add_argument(
        "--units",
        type=str,
        default="months",
    ...
    parser.add_argument(
        "--symbol", type=str, help="Filter to symbols starting with this prefix (e.g., BTC, ETH)"
    )
    parser.add_argument(
        "--suffix",
        type=str,
    ...
    parser.add_argument(
        "--plot", action="store_true", help="Generate visualization plots of ADV over time"
    )
    parser.add_argument(
        "--plot-symbols",
        type=int,
    ...
    parser.add_argument(
        "--show-all", action="store_true", help="Display all rows in output (no truncation)"
    )
    ...
    if args.suffix:
        ...
        print(
            f"   Filtered to symbols ending with '{args.suffix.upper()}': {df['symbol'].n_unique()}"
        )
    ...
    if args.symbol:
        ...
        print(
            f"   Filtered to symbols starting with '{args.symbol.upper()}': {df['symbol'].n_unique()}"
        )
    ...
    try:
        result = calculate_adv(
            df,
            args.interval,
            args.units,
            args.start_of_month,
            args.index_start_day,
            args.nsymbols,
            args.drop_n,
        )
    except Exception as e:
    ...
    if args.nsymbols:
        if args.drop_n > 0:
            print(
                f"   Filtered to ranks {args.drop_n + 1}-{args.nsymbols} (dropped top {args.drop_n})"
            )
        else:
    ...
    if args.plot:
        ...
        try:
            ...
            if args.nsymbols:
                weights_plot_filename = (
                    f"WEIGHTS_PLOT_{args.nsymbols}_{args.interval}_{units_suffix}{date_suffix}.png"
                )
    ...
    if args.output_dir:
        ...
        if args.nsymbols:
            if args.drop_n > 0:
                ...
                output_filename = (
                    f"WEIGHTS_{args.nsymbols}_{args.interval}_{units_suffix}{date_suffix}.pq"
                )
        else:
