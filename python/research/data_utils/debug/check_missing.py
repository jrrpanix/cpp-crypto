...
def get_missing_months(dates: List[datetime]) -> List[Tuple[str, str]]:
    ...
    for i in range(len(dates) - 1):
        ...
        if gap_days > 1:
            ...
            gaps.append(
                (gap_start.strftime("%Y-%m-%d"), gap_end.strftime("%Y-%m-%d"), gap_days - 1)
            )
...
def get_missing_data(aggregate_file: Path, symbol_filter: str = None) -> dict:
    ...
    for symbol in symbols:
        ...
        # Extract dates
        dates = [
            dt.date() if hasattr(dt, "date") else dt for dt in symbol_df["open_time"].to_list()
        ]
        ...
        for gap_start, gap_end, gap_days in gaps:
            ...
            symbol_info["gaps"].append(
                {"start": gap_start, "end": gap_end, "days": gap_days, "months": months_diff}
            )
...
def check_missing_data(
    aggregate_file: Path, show_clean: bool = False, symbol_filter: str = None
) -> None:
    """
    ...
    for symbol in symbols_with_gaps:
        ...
        print(
            f"   Days: {info['actual_days']} / {info['expected_days']} expected ({info['actual_days']/info['expected_days']*100:.1f}%)"
        )
        ...
        for gap in info["gaps"]:
            if gap["months"] > 1:
                print(
                    f"      ⚠️  {gap['start']} to {gap['end']} ({gap['days']} days, ~{gap['months']} months)"
                )
            else:
...
Examples:
  ...
  python check_missing.py --aggregate-dir /workspace/data/klines_aggregate
    ...
    parser.add_argument("--show-clean", action="store_true", help="Also show symbols with no gaps")
    ...
    check_missing_data(
        aggregate_file=aggregate_file, show_clean=args.show_clean, symbol_filter=args.symbol
    )
