...
long long benchmark_unordered_map(const unordered_map<string, int> &umap,
...
long long benchmark_map(const map<string, int> &omap,
                        const vector<string> &keys,
                        const vector<int> &indices) {
  long long total_ns = 0;
  for (int idx : indices) {
...
long long benchmark_sorted_vector(const vector<pair<string, int>> &sorted_vec,
                                  const vector<string> &keys,
                                  const vector<int> &indices) {
  long long total_ns = 0;
  for (int idx : indices) {
    auto start = high_resolution_clock::now();
    auto it = lower_bound(
        sorted_vec.begin(), sorted_vec.end(), keys[idx],
        [](const auto &p, const string &val) { return p.first < val; });
    volatile int x =
        (it != sorted_vec.end() && it->first == keys[idx]) ? it->second : -1;
    auto end = high_resolution_clock::now();
    total_ns += duration_cast<nanoseconds>(end - start).count();
  }
  return total_ns / indices.size();
...
int main() {
  ...
  vector<pair<string, function<long long()>>> benchmarks = {
      {"unordered_map",
       [&]() { return benchmark_unordered_map(umap, keys, indices); }},
      {"map", [&]() { return benchmark_map(omap, keys, indices); }},
      {"lower_bound",
       [&]() { return benchmark_sorted_vector(sorted_vec, keys, indices); }}};
