...
def apply_detection_filters(
    ...
    *,
    ...
    df2 = df.with_columns([ret1, ret_window, ret_mid, roll_max_day, roll_up_days, roll_mean, roll_std])

    spread_pass = (
        (pl.col("ret_window") >= target) &
        (pl.col("ret_mid") >= mid_threshold)
    ).alias("spread_pass")

    cap_pass = (
        (pl.col("ret_window") >= target) &
        (pl.col("max_day_ret") <= max_single)
    ).alias("cap_pass")

    updays_pass = (
        (pl.col("ret_window") >= target) &
        (pl.col("up_days") >= up_days_required)
    ).alias("updays_pass")

    smooth_pass = (
        (pl.col("mean_ret") >= target / window) &
        (pl.col("std_ret") <= max_std)
    ).alias("smooth_pass")
    ...
    return df2.with_columns([spread_pass, cap_pass, updays_pass, smooth_pass]).with_columns([hybrid_pass])
