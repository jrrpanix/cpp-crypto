            ...
            if sg_params:
                for window, polyorder in sg_params:
                    if window > polyorder and window % 2 != 0:
                        ...
                        daily_close = daily_close.with_columns(pl.Series(name=col_name, values=sg_filtered))
                        plt.plot(daily_close["open_time"], daily_close[col_name], label=f'SG Filter (w={window}, p={polyorder})')
                    else:
                        print(f"⚠️ SG filter window must be an odd number and greater than the polyorder (w={window}, p={polyorder}). Skipping.")
            # --- END ---
