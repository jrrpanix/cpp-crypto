...
def main():
    if len(sys.argv) < 6:
        print("Usage: python ex1.py <parquet_directory> <window> <threshold> <holding_horizon> <price_type>")
        return
    ...
    for symbol_name in symbol_names_sorted:
        try:
            ...
            print(f"Processed {symbol_name}: Total PnL after fees = {summary['total_profit_after_fees']:.2f} | Running total after fees = {running_total_after_fees:.2f}")
