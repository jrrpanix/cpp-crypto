...
def plot_symbol(symbol: str, aggregate_file: Path, output_dir: Path = None, show: bool = True, dollar_volume: bool = False) -> None:
    """
    ...
    # Plot 2: Volume
    ax2.bar(dates, volumes, width=0.8, alpha=0.7, color='steelblue', edgecolor='darkblue', linewidth=0.5)
    ...
    avg_vol_str = f"${sum(volumes)/len(volumes):,.0f}" if dollar_volume else f"{sum(volumes)/len(volumes):,.0f}"
    stats_text = (
...
Examples:
  ...
  python plot_daily.py BTC --no-show --output-dir ./plots
    ...
    parser.add_argument(
        "prefix",
        type=str,
        help="Symbol prefix (e.g., BTC, ETH, DOGE)"
    )
    parser.add_argument(
        "--suffix",
        type=str,
