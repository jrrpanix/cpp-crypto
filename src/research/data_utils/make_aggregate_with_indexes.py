...
def get_date_range(df: pl.DataFrame) -> tuple[str, str]:
    ...
    start_str = min_date.strftime("%Y-%m-%d") if hasattr(min_date, 'strftime') else str(min_date)[:10]
    end_str = max_date.strftime("%Y-%m-%d") if hasattr(max_date, 'strftime') else str(max_date)[:10]
    
    return start_str, end_str


def combine_files(binance_dir: Path, index_dir: Path, output_dir: Path, start_date: str = None, end_date: str = None) -> None:
    """
    ...
    try:
        ...
        symbol_counts = combined_df.group_by("symbol").agg(pl.count().alias("count")).sort("count", descending=True)
        for row in symbol_counts.head(15).iter_rows(named=True):
            index_marker = " [INDEX]" if any(row["symbol"] in s for s in symbols_processed if "[INDEX]" in s) else ""
...
Examples:
  ...
  python make_aggregate_with_indexes.py \\
    ...
    parser.add_argument(
        "--start-date",
        type=str,
        help="Start date (YYYY-MM-DD) - optional filter"
    )
    parser.add_argument(
        "--end-date",
        type=str,
        help="End date (YYYY-MM-DD) - optional filter"
    )
