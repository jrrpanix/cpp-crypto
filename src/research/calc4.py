...
def main():
    if len(sys.argv) < 7:
        print("Usage: python calc4.py <parquet_directory> <symbol> <window> <threshold> <holding_horizon> <price_type>")
        return
    ...
    try:
        trade_df, summary = symbol_trade_move(parquet_dir, symbol, window, threshold, holding_horizon, price_type)
    ...
    print(trade_df)
    print("\nTrade summary:")
    print(f"Total rows: {summary['summary_stats']['total_rows']} | Date range: {summary['summary_stats']['min_date']} to {summary['summary_stats']['max_date']}")
    print(f"Starting price: {summary['summary_stats'].get('open_price')} occurs on date {summary['summary_stats'].get('open_price_date')}")
    print(f"Min price (low): {summary['summary_stats'].get('min_price')} occurs on date {summary['summary_stats'].get('min_price_date')}")
    print(f"Max price (high): {summary['summary_stats'].get('max_price')} occurs on date {summary['summary_stats'].get('max_price_date')}")
    print(f"Ending price: {summary['summary_stats'].get('close_price')} occurs on date {summary['summary_stats'].get('close_price_date')}")
