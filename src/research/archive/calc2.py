...
import polars as pl
from patterns import summary, windowed_up_down_probabilities
...
def main():
    ...
    probs = windowed_up_down_probabilities(
        df, price_col="close", threshold=threshold, window=window, lookaheads=lookaheads
    )
    ...
    for direction in ["up", "down"]:
        ...
        for look in lookaheads:
            ...
            print(
                f"  {look} windows ahead: up={prob_up*100:.2f}%, down={prob_down*100:.2f}%, total profit/loss=${profit:.2f} (for $1000 per trade)"
                + (f", n_events={n_events}" if n_events is not None else "")
            )
        print()
